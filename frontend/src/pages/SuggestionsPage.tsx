...
export const SuggestionsPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useSuggestions({ page: 1, page_size: 50, codebase_id: codebaseId || undefined });
  ...
  if (!suggestions || suggestions.length === 0) {
    return (
      <Box sx={{ p: 3, bgcolor: '#0F1115', minHeight: '100vh' }}>
        <PageHeader
          title="Suggestions"
          subtitle={selectedProject ? `Suggestions for ${selectedProject.name}` : "Improvement recommendations"}
          onRefresh={() => refetch()}
        />
        <EmptyState
          variant="no-data"
          ...
          message={selectedProject 
            ? "No optimization suggestions have been generated yet for this project. Run analysis to populate suggestions."
