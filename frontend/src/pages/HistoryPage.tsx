...
export const HistoryPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useAnalysisRuns({ page: 1, page_size: 50, codebase_id: codebaseId || undefined });
    ...
    return (
      <Box sx={{ p: 3, bgcolor: '#0F1115', minHeight: '100vh' }}>
        <PageHeader
          ...
          subtitle={selectedProject ? `History for ${selectedProject.name}` : "Historical scan results"}
          onRefresh={() => refetch()}
        />
        <EmptyState
          variant="no-data"
          ...
          message={selectedProject 
            ? "No analysis runs have been performed yet for this project. Run analysis to populate history."
            : "Select a project to view analysis history."
          }
        />
      </Box>
    );
