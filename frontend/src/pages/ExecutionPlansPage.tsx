...
export const ExecutionPlansPage: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useExecutionPlans({ page: 1, page_size: 50, codebase_id: codebaseId || undefined });
    ...
    return (
      <Box sx={{ p: 3, bgcolor: '#0F1115', minHeight: '100vh' }}>
        <PageHeader
          ...
          subtitle={selectedProject ? `Execution Plans for ${selectedProject.name}` : "PostgreSQL EXPLAIN analysis results"}
          onRefresh={() => refetch()}
        />
        <EmptyState
          variant="no-data"
          ...
          message={selectedProject 
            ? "No execution plans have been generated yet for this project. Run analysis to populate execution plans."
            : "Select a project to view execution plans."
          }
        />
      </Box>
    );
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#0F1115', minHeight: '100vh' }}>
      <PageHeader
        title="Execution Plans"
        subtitle={`Found ${data?.total || 0} PostgreSQL EXPLAIN results`}
        onRefresh={() => refetch()}
      />
